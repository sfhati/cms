...
function SYNTAX_menu($template) {
    $menu_var = chk_var($template);
    $TEMPLATE_link = TEMPLATE_PATH . 'sfhati/';
    $template = SYNTAX_EASY($menu_var[0]);
    $pag_id = 0;
    if ($menu_var[1]) {
        $mnu_clss = SYNTAX_EASY($menu_var[1]);
    } else {
        $mnu_clss = 'current active';
    }
    if ($menu_var[2]) {
        $pag_id = SYNTAX_EASY($menu_var[2]);
    }
    if ($menu_var[3]) {
        if (SYNTAX_EASY($menu_var[3]) == 'parent') {
            if($pag_id == get_root_page_id($pag_id)) return ''; // its not slave page , its parent!
            $pag_id = get_parent_page_id($pag_id);
        }
        if (SYNTAX_EASY($menu_var[3]) == 'root') {
            $pag_id = get_root_page_id($pag_id);
        }
    }
    return get_pages_under_Tpage_id($template, $pag_id, "page_place like '%$template%' and ", $mnu_clss);
}
