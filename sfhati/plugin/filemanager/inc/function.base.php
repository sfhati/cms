...
return $_SESSION['filemanagersfhati'];
    $roorPath = $_SESSION['filemanagersfhati'];
    ...
    $roorPath = $_SESSION['filemanagertemplates'];
    if (file_exists($value) && @strpos(strtolower(addTrailingSlash(backslashToSlash(getRealPath($value)))), $roorPath) === 0) {
        return true;
    }
    $roorPath = $_SESSION['filemanageruploaded'];
    if (file_exists($value) && @strpos(strtolower(addTrailingSlash(backslashToSlash(getRealPath($value)))), $roorPath) === 0) {
        return true;
    }
    return false;
...
function getFolderListing($path, $indexNumber = null, $prefixNumber = ' ', $prefixName = ' - ', $outputs = array()) {
    ...
    if ($fh) {
        ...
        while ($file = @readdir($fh)) {
            ...
            if (isListingDocument($newPath) && $file != '.' && $file != '..' && $file != '.svn' && is_dir($newPath)) {
