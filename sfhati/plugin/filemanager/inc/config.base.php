...
//error_reporting(E_ALL ^ E_NOTICE);	
	...
	//FILESYSTEM CONFIG
		...
		*/
	define('CONFIG_SYS_DEFAULT_PATH',$_SESSION['uploaded']); //accept relative path only
	define('CONFIG_SYS_ROOT_PATH', $_SESSION['uploaded']);	//accept relative path only
	...
	define('CONFIG_EDITABLE_VALID_EXTS', 'inc,txt,htm,html,xml,js,css'); //make you include all these extension in CONFIG_UPLOAD_VALID_EXTS if you want all valid
	
	define('CONFIG_OVERWRITTEN', true); //overwirte when processing paste
	define('CONFIG_UPLOAD_VALID_EXTS', 'gif,jpg,png,bmp,tif,zip,sit,inc,rar,gz,js,css,tar,rm,htm,html,mov,mpg,avi,asf,mpeg,wmv,aif,aiff,wav,mp3,swf,ppt,rtf,doc,pdf,xls,txt,xml,xsl,dtd');//
	define("CONFIG_VIEWABLE_VALID_EXTS", 'gif,bmp,inc,txt,jpg,png,tif,html,htm,js,css,xml,xsl,dtd,mp3,wav,wmv,wma,rm,rmvb,mov,swf');
	...
	define('CONFIG_THICKBOX_MAX_HEIGHT', 430);
		 ...
		 * 			?>
		 *   3. upload document_root.php to you website root folder which will only be reached when you visit http://www.domain-name.com or / at localhost computer
		 *   4. run it via http://www.domain-name.com/document_root.php or /docuent_root.php if localhost computer, the url has to be exactly like that 
	...
	define('CONFIG_LANG_DEFAULT', (CONFIG_QUERY_STRING_ENABLE && !empty($_GET['language']) && file_exists(DIR_LANG . secureFileName($_GET['language']) . '.php')?secureFileName($_GET['language']):$_SESSION['lng_CH'])); //change it to be your language file base name, such en
?>